...
const BarberLogin: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Verifica role e status na tabela profiles (fonte de verdade — nunca confiar em user_metadata)
  ...
  return (
    <div className="dark">
      <div className="bg-background-dark text-slate-100 font-sans min-h-screen flex flex-col items-center justify-center p-4">
      <div className="mb-8 flex items-center gap-3">
        <RazorIcon className="w-12 h-12 text-slate-200 drop-shadow-xl" />
        <h1 className="text-xl font-medium tracking-tight uppercase text-white uppercase italic">BARBER <span className="text-primary">KING</span></h1>
      </div>
      <div className="w-full max-w-md bg-card-dark border border-white/10 rounded-2xl shadow-2xl p-8">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-medium text-white mb-2">Portal do Barbeiro</h2>
          <p className="text-slate-400 text-sm">Acesso restrito aos profissionais</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-6">
          <div className="space-y-2">
            <label htmlFor="email" className="text-xs font-medium tracking-[0.2em] uppercase text-slate-400 ml-1">E-mail Profissional</label>
          ...
          <div className="space-y-2">
            <label htmlFor="password" className="text-xs font-medium tracking-[0.2em] uppercase text-slate-400 ml-1 text-left block">Senha</label>
            <div className="relative">
          ...
          <button type="submit" disabled={loading} className="w-full bg-primary hover:bg-primary/90 text-white font-medium py-4 rounded-xl shadow-lg shadow-primary/20 transition-all transform active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? 'Verificando...' : 'ENTRAR'}
            <span className="material-symbols-outlined text-lg">login</span>
          </button>
          <div className="flex justify-end mt-2">
            <a href="#" className="text-sm text-slate-400 hover:text-white transition-colors">Esqueceu a senha?</a>
          </div>
        </form>
        <div className="mt-8 pt-6 border-t border-white/5 text-center space-y-4">
          <div className="flex justify-center items-center text-sm px-2">
             <button 
                onClick={() => navigate('/barbeiro/register')}
          ...
          <button 
            onClick={() => navigate('/')} 
            className="inline-flex items-center gap-2 text-xs font-medium tracking-[0.2em] uppercase text-slate-500 hover:text-white transition-all"
          >
